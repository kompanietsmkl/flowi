...
export const Item = (
    ...
    return (
        <div
            ...
            className={cn("group min-h-[25px] text-sm py-[6px] pr-3 w-full",
            "hover:bg-primary/5 flex items-center rounded-lg text-muted-foreground font-medium",
            active && "bg-primary/5")}
        >
            {!!id && (
                <div role="button"
            ...
            {isSearch &&(
                ...
                select-none items-center gap-1 rounded border px-1.5 text-[10px]
...
Item.Skeleton = function ItemSkeleton ({ level = 0 }:{level?:number}) {
    return (
        <div style={{
            paddingLeft: level? `${(level * 10) + 15}px`: "12px"
            }}
